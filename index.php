...
<!--###########################################################################################################################################################-->
 <?php
	...
	if(isset($_POST['register']))
	{
		...
		//###################################################################################################################
		//Werte: Vorname, Nachname, Job Titel ,Email, Firma, PLZ, Ort, Straße, Hausnummer, Telefonnummer, Mobil, email, fqdn, time, accept_terms_of_condition, kundennummer(uuid), register_email_link, accepted_by_email,datensatz_erstellt_am, datensatz_gaendert_am, geaendert_von_wem
		...
		//echo guidv4(openssl_random_pseudo_bytes(16)); 	//bis PHP 5
		//echo guidv4(random_bytes(16)); 					//ab PHP 7
		$kundennummer_uuid = guidv4(random_bytes(16));
		//###############################################
		
		//Zuffalsstring 50 Zeichen generieren###############	
		...
		$zufalls_string = generateRandomString();
		//$register_email_link = "http://rocketsearch.ddns.net/registermail.php?".$zufalls_string;
		$register_email_link = "http://register.ddns.net/registermail.php?".$zufalls_string;
		//###############################################
		...
		//SQL Query senden#################################################
		//$sql = "INSERT INTO double_opt_in (vorname, nachname, email, jobtitel, telefon, mobil, firma, plz, ort, strasse, hausnummer, accept_terms_of_condition, ip, fqdn, kundennummer_uuid, register_email_link, accepted_by_email ) VALUES ('".$_POST["vorname"]."','".$_POST["nachname"]."','".$_POST["email"]."','".$_POST["jobtitel"]."','".$_POST["telefon"]."','".$_POST["mobil"]."', '".$_POST["firma"]."','".$_POST["plz"]."','".$_POST["ort"]."','".$_POST["strasse"]."','".$_POST["hausnummer"]."','".$_POST["terms"]."','IP','fqdn','kundennummer_uuid','register_email_link','accepted_by_email')";
		$sql = "INSERT INTO double_opt_in (vorname, nachname, email, jobtitel, telefon, mobil, firma, plz, ort, strasse, hausnummer, accept_terms_of_condition, ip, fqdn, kundennummer_uuid, register_email_link, accepted_by_email,datensatz_erstellt_am, datensatz_gaendert_am, geaendert_von_wem ) VALUES ('".$_POST["vorname"]."','".$_POST["nachname"]."','".$_POST["email"]."','".$_POST["jobtitel"]."','".$_POST["telefon"]."','".$_POST["mobil"]."', '".$_POST["firma"]."','".$_POST["plz"]."','".$_POST["ort"]."','".$_POST["strasse"]."','".$_POST["hausnummer"]."','".$_POST["terms"]."','".$ipaddress."','".$fqdn."','".$kundennummer_uuid."','".$register_email_link."','NO','".$datensatz_erstellt_am."','datensatz_gaendert_am','index.php; first_register_before_email')";
		//echo "sql# ".$sql;
		//echo "<br>";
		...
		$betreff = '=?UTF-8?B?'.base64_encode('Registrierung bei rocketsearch').'?=';
		...
		$msg     = '<html><head><title>Registrierung bei rocketsearch</title></head>';
		$msg    .= '<body><p>Hallo '.$_POST["vorname"].' '.$_POST["nachname"].'</p>Bitte auf folgenden Link klicken, um die Registrierung abzuschliesen <br/><br/> <a href="'.$register_email_link.'">register link</a>  <br/><br/> Danke  </body></html>';
		 // Mail versenden
...
<form action="index.php" method="post" accept-charset="utf-8" onsubmit="return checkForm(this);"> 
